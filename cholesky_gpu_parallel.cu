...
__global__ void device_cholesky_normalize(int n, Matrix A, int k);
__global__ void device_cholesky_submat_update(int n, Matrix A, int k);
__global__ void device_cholesky_lower_traingale_update(int n, Matrix A, int k);
...
//Normalizing the elemnts in the pivot row k from column k
__global__ void device_cholesky_normalize(int n, Matrix A, int k) {
    int i = k + blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) {
        A.array[k * n + i] /= sqrt(A.array[k * n + k]);
    }
}

//Rank 1 update on submatrix starting at (k+1, k+1)
__global__ void device_cholesky_submat_update(int n, Matrix A, int k) {
    int i = blockIdx.y * blockDim.y + threadIdx.y + k + 1;
    int j = blockIdx.x * blockDim.x + threadIdx.x + k + 1;
    if (i < n && j < n) {
        A.array[i * n + j] -= A.array[k * n + i] * A.array[k * n + j];;
    }
}

//update the elements below the diagonal in column k to zero
__global__ void device_cholesky_lower_traingale_update(int n, Matrix A, int k) {
...
//parallel Cholesky factorization on the GPU
void parallel_cholesky_factorization(int n, Matrix A, int block_1d_grid, int block_2d_grid_x, int block_2d_grid_y) {
    cudaError_t err=cudaSuccess;
    for (int pivot = 0; pivot < n; pivot++) {
        int grid_normalize = (n - pivot + block_1d_grid - 1) / block_1d_grid;
        //normalizing row k starting from k colum by dividing with square root of pivot element.
        device_cholesky_normalize<<<grid_normalize, block_1d_grid>>>(n, A, pivot);
        err = cudaGetLastError();
        check_error(err, ERR_KERNEL);

        int dimension = n - (pivot + 1);
        if (dimension > 0) {
            //2D grid for rank 1 update on submatrix
            dim3 block2D(block_2d_grid_x, block_2d_grid_y);
            dim3 grid2D((dimension + block_2d_grid_x - 1) / block_2d_grid_x, (dimension + block_2d_grid_y - 1) / block_2d_grid_y);
            device_cholesky_submat_update<<<grid2D, block2D>>>(n, A, pivot);
            err = cudaGetLastError();
            check_error(err, ERR_KERNEL);

            //setting the elements of lower triangle in column k region to 0.
            int gride_lower_triangle = (dimension + block_1d_grid - 1) / block_1d_grid;
            device_cholesky_lower_traingale_update<<<gride_lower_triangle, block_1d_grid>>>(n, A, pivot);
            err = cudaGetLastError();
            check_error(err, ERR_KERNEL);
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 5) {
        ...
        printf("Use: <executable_name> <matrix_size> <block_1d_grid> <block_2d_grid_x> <block_2d_grid_y>\n");
        exit(0);
    }
    ...
    int block_1d_grid = atoi(argv[2]);
    if (block_1d_grid <= 0 || block_1d_grid > 1024) {
        printf("Block size in 1d grid must be between 1 and 1024.\n");
        exit(0);
    }
    int block_2d_grid_x = atoi(argv[3]);
    int block_2d_grid_y = atoi(argv[4]);
    if(block_2d_grid_x <=0 || block_2d_grid_y <=0 || block_2d_grid_x * block_2d_grid_y> 1024){
        printf("Product of x and y dimensions in 2d grid must be less than 1024.\n");
        exit(0);
    }
    ...
    parallel_cholesky_factorization(dA.n, dA,block_1d_grid,block_2d_grid_x,block_2d_grid_y);
    err = cudaGetLastError(); check_error(err, ERR_KERNEL);
